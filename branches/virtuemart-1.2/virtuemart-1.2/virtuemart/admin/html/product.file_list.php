...
$listObj->writeSearchHeader($VM_LANG->_('VM_FILES_LIST') ." " . $product_name, $mosConfig_live_site."/administrator/images/mediamanager.png", $modulename, "file_list");
...
$columns = Array(  "#" => 'width="20"', 
					...
					$VM_LANG->_('VM_FILES_LIST_FILENAME') => '',
					$VM_LANG->_('VM_FILES_LIST_ROLE') => '',
					$VM_LANG->_('VM_VIEW') => '',
					$VM_LANG->_('VM_FILES_LIST_FILETITLE') => '',
					$VM_LANG->_('VM_FILES_LIST_FILETYPE') => '',
...
while ($db->next_record()) {
	...
	$tmp_cell = "<a href=\"".$sess->url( $_SERVER['PHP_SELF'].'?page=product.file_form&amp;product_id='.$product_id.'&amp;file_id='.$db->f("file_id")).'&amp;no_menu='.@$_REQUEST['no_menu'].'" title="'.$VM_LANG->_('VM_MANUFACTURER_LIST_ADMIN').'">';
	$style = '';
	if($filename) {
	...
	if( $db->f("file_is_image")) {
		...
		if( is_file( $fullimg ) ) {
			$tmp_cell .= $VM_LANG->_('VM_FILES_LIST_FULL_IMG').": ";
			$tmp_cell .= '<a onclick="document.getElementById(\'file_form_iframe\').src=\''.$db->f("file_url") . '\';" href="#file_form" title="Click me!">[ '.$VM_LANG->_('VM_VIEW') . ' ]</a>'; 
		}
		$tmp_cell .= '<br />';
		...
		if( is_file( $thumb ) ) {
			$tmp_cell .= $VM_LANG->_('VM_FILES_LIST_THUMBNAIL_IMG').": ";
			$tmp_cell .= vmToolTip( '&nbsp;<img src="'.$thumburl.'" alt="thumbnail" />', $VM_LANG->_('VM_FILES_LIST_THUMBNAIL_IMG'), '', '', '[ '.$VM_LANG->_('VM_VIEW') . ' ]' ); 
		}
		if( !$db->f("file_name") ) {
			$tmp_cell = "&nbsp;<a target=\"_blank\" href=\"".$db->f("file_url"). "\">[ ".$VM_LANG->_('VM_VIEW') . " ]</a><br/>"; 
		}
	}
	$listObj->addCell( $tmp_cell );
