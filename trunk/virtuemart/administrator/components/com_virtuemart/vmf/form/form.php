...
class vForm extends JForm{
	...
	{
		// Make sure there is a valid vForm XML document.
		...
		if (is_object($data))
		{
			...
			elseif ($data instanceof vObject)
			{
				// Handle a JObject.
	...
	{
		JLog::add('vForm->getControlGroups() is deprecated use vForm->renderFieldset().', JLog::WARNING, 'deprecated');
	...
	public function loadFile($file, $reset = true, $xpath = false)
	{
		...
		if (!is_file($file))
		{
			...
			$file = vPath::find(self::addFormPath(), strtolower($file) . '.xml');
	 ...
	 *
	 * Validation warnings will be pushed into vForm::errors and should be
	 * retrieved with vForm::getErrors() when validate returns boolean false.
	 *
		...
		switch (strtoupper($filter))
		{
			...
			case 'SERVER_UTC':
				if ((int) $value > 0)
				{
					...
					$offset = vFactory::getConfig()->get('offset');

					// Return an SQL formatted datetime string in UTC.
					$return = vFactory::getDate($value, $offset)->toSql();
				}
				else
				{
					$return = '';
			...
			case 'USER_UTC':
				if ((int) $value > 0)
				{
					...
					$offset = vFactory::getUser()->getParam('timezone', vFactory::getConfig()->get('offset'));
			...
			case 'URL':
				...
				{
					$host = vUri::getInstance('SERVER')->gethost();
				...
				$value = vStringPunycode::urlToPunycode($value);
				$return = $value;
				break;
	...
	protected function loadField($element, $group = null, $value = null)
	{
		...
		// Load the vFormField object for the field.
		...
		return $this;');
			...
			//$field = $this->objectToObject($field,$newName);
		} else {
			//$field->setForm((JFormField)$this);
		}
		$field->setForm($this);
	...
	/**
	 * Proxy for {@link vFormHelper::loadFieldType()}.
	 *
	...
	{
		return vFormHelper::loadFieldType($type, $new);
	}

	/**
	 * Proxy for vFormHelper::loadRuleType().
	 *
	 ...
	 *
	 * @see     vFormHelper::loadRuleType()
	 * @since   11.1
	 */
	protected function loadRuleType($type, $new = true)
	{
		return vFormHelper::loadRuleType($type, $new);
	}
	...
	protected function validateFieldVM(SimpleXMLElement $element, $group = null, $value = null, Registry $input = null) {
		...
		if ($required)
		{
			...
			if (($value === '') || ($value === null))
			{
				if ($element['label'])
				{
					$message = vmText::_($element['label']);
				}
				else
				{
					$message = vmText::_($element['name']);
				}

				$message = vmText::sprintf('JLIB_FORM_VALIDATE_FIELD_REQUIRED', $message);
		...
		if ($valid === false)
		{
			...
			if ($message)
			{
				$message = vmText::_($element['message']);
			...
			{
				$message = vmText::_($element['label']);
				$message = vmText::sprintf('JLIB_FORM_VALIDATE_FIELD_INVALID', $message);
	...
	{
		return vFormHelper::addFieldPath($new);
	}

	/**
	 * Proxy for vFormHelper::addFormPath().
	 *
	 * @param   mixed  $new  A path or array of paths to add.
	 *
	 * @return  array  The list of paths that have been added.
	 *
	 * @see     vFormHelper::addFormPath()
	 * @since   11.1
	 */
	public static function addFormPath($new = null)
	{
		return vFormHelper::addFormPath($new);
	}

	/**
	 * Proxy for vFormHelper::addRulePath().
	 *
	 * @param   mixed  $new  A path or array of paths to add.
	 *
	 * @return  array  The list of paths that have been added.
	 *
	 * @see     vFormHelper::addRulePath()
	 * @since   11.1
	 */
	public static function addRulePath($new = null)
	{
		return vFormHelper::addRulePath($new);
	}
	...
	public static function getInstance($name, $data = null, $options = array(), $replace = true, $xpath = false)
	{
		...
		if (!isset($forms[$name]))
		{
			...
			if (empty($data))
			{
				throw new InvalidArgumentException(sprintf('vForm::getInstance(name, *%s*)', gettype($data)));
			}

			// Instantiate the form.
			$forms[$name] = new vForm($name, $options);
			...
			if (substr(trim($data), 0, 1) == '<')
			{
				...
				{
					throw new RuntimeException('vForm::getInstance could not load form');
				}
			}
			else
			{
				...
				{
					throw new RuntimeException('vForm::getInstance could not load file');
				}
			}
		}
