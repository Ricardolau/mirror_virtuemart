...
if ($cart["idx"] == 0) {
	...
	for ($i=0;$i<$cart["idx"];$i++) {
        ...
        <input type=\"image\" name=\"update\" title=\"". $VM_LANG->_('VM_CART_UPDATE') ."\" src=\"". IMAGEURL ."ps_image/edit_f2.gif\" value=\"". $VM_LANG->_('VM_UPDATE') ."\" />
      </form>";
        ...
        <input type=\"hidden\" name=\"product_id\" value=\"". $_SESSION['cart'][$i]["product_id"] ."\" />
        <input type=\"hidden\" name=\"description\" value=\"". $cart[$i]["description"]."\" />
      <input type=\"image\" name=\"delete\" title=\"". $VM_LANG->_('VM_CART_DELETE') ."\" src=\"". IMAGEURL ."ps_image/delete_f2.gif\" value=\"". $VM_LANG->_('VM_CART_DELETE') ."\" />
      </form>";
	...
	if ($_REQUEST["page"] == "checkout.index" && !empty($_POST["do_coupon"])) {
		...
		if (@$_SESSION['coupon_redeemed'] == true) {
			$vmLogger->warning( $VM_LANG->_('VM_COUPON_ALREADY_REDEEMED',false) );
		}
		else {
	...
	if ( PAYMENT_DISCOUNT_BEFORE == '1') {
		if( $payment_discount != 0.00 ) {
			...
			if( $payment_discount > 0.00 ) {
				$discount_word = $VM_LANG->_('VM_PAYMENT_METHOD_LIST_DISCOUNT');
			}
			else {
				$discount_word = $VM_LANG->_('VM_FEE');
			}
