...
if ($db->next_record()) {
	...
	$mainframe->setPageTitle( $VM_LANG->_('VM_ACC_ORDER_INFO').' : '.$VM_LANG->_('VM_ORDER_LIST_ID').' '.$db->f('order_id'));
	require_once( CLASSPATH.'ps_product_category.php');
	
	// Set the CMS pathway
	$pathway = array();
	$pathway[] = $vm_mainframe->vmPathwayItem( $VM_LANG->_('VM_ACCOUNT_TITLE'), $sess->url( SECUREURL .'index.php?page=account.index' ) );
	$pathway[] = $vm_mainframe->vmPathwayItem( $VM_LANG->_('VM_ACC_ORDER_INFO') );
