...
<h4><?php echo $VM_LANG->_('VM_REVIEWS') ?>:</h4>
...
foreach( $reviews as $review ) { // Loop through all reviews
	...
	<br />
	<?php echo $VM_LANG->_('VM_RATE_NOM') // "Rating:" ?>: 
...
if( $num_rows < 1 ) {
  echo $VM_LANG->_('VM_NO_REVIEWS')." <br />"; // "There are no reviews for this product"
  if (!empty($my->id)) {
  	echo $VM_LANG->_('VM_WRITE_FIRST_REVIEW'); // "Be the first to write a review!"
  }
  else {
  	echo $VM_LANG->_('VM_REVIEW_LOGIN'); // Login to write a review!
  }
}
