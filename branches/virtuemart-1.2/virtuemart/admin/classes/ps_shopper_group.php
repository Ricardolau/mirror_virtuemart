...
//		$vendor_id = $_SESSION["ps_vendor_id"];
		...
		if (empty($d["shopper_group_name"])) {
			$GLOBALS['vmLogger']->err($VM_LANG->_('VM_SHOPPER_GROUP_MISSING_NAME'));
			return False;
		}
		else {
...
//			$q .= "AND `vendor_id`='" . $vendor_id . "'";
			...
			$db->next_record();
			if ($db->f("num_rows") > 0) {
				$GLOBALS['vmLogger']->err($VM_LANG->_('VM_SHOPPER_GROUP_ALREADY_EXISTS'));
				return False;
			}
		}
// Change this
	...
	function validate_delete( $shopper_group_id, &$d) {
		...
		if (empty($shopper_group_id)) {
			$GLOBALS['vmLogger']->err($VM_LANG->_('VM_SHOPPER_GROUP_DELETE_SELECT'));
			return False;
		}
		...
		if( $db->next_record()) {
			$GLOBALS['vmLogger']->err(str_replace('{id}',$shopper_group_id,$VM_LANG->_('VM_SHOPPER_GROUP_DELETE_PAYMENT_METHODS_ASS')));
			return False;
		}
		...
		if( $db->next_record()) {
			$GLOBALS['vmLogger']->err(str_replace('{id}',$shopper_group_id,$VM_LANG->_('VM_SHOPPER_GROUP_DELETE_USERS_ASS')));
			return False;
		}
		...
		$db->query($q);
		if ($db->next_record()) {
			$GLOBALS['vmLogger']->err($VM_LANG->_('VM_SHOPPER_GROUP_DELETE_DEFAULT'));
			return False;
		}
		...
		*/
		...
		if (empty($shopper_group_id)) {	// INSERT
			...
			if( $db->query() !== false ) {
				...
				$vmLogger->info($VM_LANG->_('VM_SHOPPER_GROUP_ADDED'));
				$answer = $_REQUEST['shopper_group_id'];
			} else {
				$vmLogger->err($VM_LANG->_('VM_SHOPPER_GROUP_ADD_FAILED'));
			}
		} else { // UPDATE
			$db->buildQuery( 'UPDATE', '#__{vm}_shopper_group', $fields, 'WHERE `shopper_group_id`=' . (int)$d["shopper_group_id"] );

			if( $db->query() ) {
				$GLOBALS['vmLogger']->info($VM_LANG->_('VM_SHOPPER_GROUP_UPDATED'));

				$answer = True;
			} else {
				$GLOBALS['vmLogger']->err($VM_LANG->_('VM_SHOPPER_GROUP_UPDATE_FAILED'));
			}
			if($answer) {
				if ($default == "1") {
