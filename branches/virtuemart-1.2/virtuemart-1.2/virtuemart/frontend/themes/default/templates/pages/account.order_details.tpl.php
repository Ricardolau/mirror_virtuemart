...
?>	
	...
	if ($db->f("order_status") == ENABLE_DOWNLOAD_STATUS && ENABLE_DOWNLOADS) {
		...
		if ($dbdl->next_record()) {
			echo "<b>" . $VM_LANG->_('VM_DOWNLOADS_CLICK') . "</b><br /><br />";
	
			echo($VM_LANG->_('VM_DOWNLOADS_SEND_MSG_3').DOWNLOAD_MAX.". <br />");
	
			$expire = ((DOWNLOAD_EXPIRE / 60) / 60) / 24;
			echo(str_replace("{expire}", $expire, $VM_LANG->_('VM_DOWNLOADS_SEND_MSG_4')));
			
			echo "<br /><br />";
		}
		//else {
			//echo "<b>" . $VM_LANG->_('VM_DOWNLOADS_EXPIRED') . "</b><br /><br />";
		//}
	}
	?>
	    </td>
	          ...
	          <td><?php 
	              if ($dbdl->next_record()) {
	        			...
	        			echo '<a href="'."$url&download_id=".$dbdl->f("download_id").'">'
	        					. '<img src="'.VM_THEMEURL.'images/download.png" alt="'.$VM_LANG->_('VM_DOWNLOADS_CLICK').'" align="left" border="0" />&nbsp;'
	...
	?>      <tr> 
	        ...
	        <tr> 
	          ...
	          <strong><?php echo $VM_LANG->_('VM_CART_TOTAL') .": "; ?></strong>
	          </td>
