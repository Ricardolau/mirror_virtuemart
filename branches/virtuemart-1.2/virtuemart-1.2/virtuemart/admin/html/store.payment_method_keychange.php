...
if ( $payment_method_id ) {
	...
	echo "<h2>".$VM_LANG->_('VM_CHANGE_PASSKEY_FORM')."</h2></th>";
	echo '</tr><tr><td>';
	...
	if( $auth_result && empty( $passkey ) ) {
		...
		echo "<table width=\"60%\">\n";
		echo "<tr><th style=\"text-align:right;\">".$VM_LANG->_('VM_CURRENT_TRANSACTION_KEY').":</th>\n";
		echo "<th><input type=\"text\" name=\"passkey\" value=\"". $db->f('passkey') ."\" /></th></tr>\n";
		echo "<tr><td>&nbsp;</td><td>&nbsp;</td></tr>\n";
		echo "<tr><td style=\"text-align:right;\">".$VM_LANG->_('VM_TYPE_PASSWORD').":</td>\n";
		echo "<td><input type=\"password\" name=\"passwd\" value=\"\" /></td></tr>\n";
		echo "<tr><td>&nbsp;</td><td>&nbsp;</td></tr>\n";
		echo "<tr><td>&nbsp;</td><td><input name=\"submit\" type=\"submit\" value=\"".$VM_LANG->_('VM_SUBMIT')."\" /></td></tr>\n";
		echo "</table>\n";
	...
	elseif ( $auth_result && !empty($passkey) && $task == "changekey") {
		...
		$db->query( $q );
		vmRedirect( $sess->url($_SERVER['PHP_SELF']."?page=store.payment_method_form&payment_method_id=$payment_method_id", false, false), $VM_LANG->_('VM_CHANGE_PASSKEY_SUCCESS'));
	}
	// not authenticated
	else {
		...
		echo "<tr><td>".$VM_LANG->_('VM_CURRENT_TRANSACTION_KEY').":</td><td>".( $db->f('passkey') ? ps_checkout::asterisk_pad( $db->f('passkey'), 4 ) : '<i>(empty!)</i>')."</td></tr>\n";
		echo "<tr><td>&nbsp;</td><td>&nbsp;</td></tr>\n";
		echo "<tr><td>".$VM_LANG->_('VM_TYPE_PASSWORD').":</td>\n";
		echo "<td><input type=\"password\" name=\"passwd\" value=\"\" /></td></tr>\n";
		echo "<tr><td>&nbsp;</td><td>&nbsp;</td></tr>\n";
		echo "<tr><td>&nbsp;</td><td><input name=\"submit\" type=\"submit\" value=\"".$VM_LANG->_('VM_SUBMIT')."\" /></td></tr>\n";
		echo "</table>\n";
