...
/**
*
* @version $Id: ps_cart.php 1760 2009-05-03 22:58:57Z Aravot $
* @package VirtueMart
* @subpackage classes
...
//		$GLOBALS['vmLogger']->info( 'add '.$d["product_id"].'$cart_vendor_id '.$cart_vendor_id);
		...
		for ($ikey = 0; $ikey < $multiple_products; $ikey++) {
			...
			if ($quantity < 0) {
				$vmLogger->warning( $VM_LANG->_('VM_CART_ERROR_NO_NEGATIVE',false) );
				return False;
			}

			if (!ereg("^[0-9]*$", $quantity)) {
				$vmLogger->warning( $VM_LANG->_('VM_CART_ERROR_NO_VALID_QUANTITY',false) );
				return False;
			}
			...
			if ( $db->num_rows()) {
				$vmLogger->tip( $VM_LANG->_('VM_CART_SELECT_ITEM',false) );
				return false;
			}
...
//		$_SESSION['cart'] = $cart;
		...
		if($total_quantity !=0 || $total_updated !=0 || $total_deleted !=0) {
            ...
            $vmLogger->tip( $VM_LANG->_('VM_CART_GOTO_WAITING_LIST',false) );
		} else {
            $vmLogger->tip( $VM_LANG->_('VM_CART_QUANTITY_EXCEEDED',false) );
        }
