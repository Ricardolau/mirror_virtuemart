<?php if( !defined( '_JEXEC' ) ) die( 'Direct Access to '.basename(__FILE__).' is not allowed.' ); ?>
	...
	if (($product_in_stock < 1 && CHECK_STOCK) || $product_available_date > time() ) { 
		...
		<?php echo $VM_LANG->_('VM_CURRENTLY_NOT_AVAILABLE') ?>
		<br />
	...
	if($product_available_date > time()) { ?>
			<?php 
			echo $VM_LANG->_('VM_PRODUCT_AVAILABLE_AGAIN')
	...
	elseif( ($product_in_stock >= 1 && CHECK_STOCK) ) {
		?><span style="font-weight:bold;">
			<?php echo $VM_LANG->_('VM_PRODUCT_FORM_IN_STOCK') ?>: 
	...
	if( $product_availability ) { ?>
		<span style="font-weight:bold;">
			<?php echo $VM_LANG->_('VM_DELIVERY_TIME') ?>: 
		</span>
		<br /><br />
		<?php
	...
	if( !empty( $avail ) ) { 
		?>
		<div class="availabilityHeader"><?php echo $VM_LANG->_('VM_AVAILABILITY') ?></div>
		<br />
		<?php
		echo $avail;
	}
