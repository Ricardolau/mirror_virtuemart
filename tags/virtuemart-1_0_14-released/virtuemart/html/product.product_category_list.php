...
if( $keyword != '' ) {
	$j = 0 ;
	for( $i = 0 ; $i < $size ; $i ++ ) {
		if( stristr( $categories[$key[$i]]["category_name"], $keyword ) ) {
			$category_tmp[$j] = &$categories[$key[$i]] ;
			$j ++ ;
		}
	}
} else {
...
$parent_ids = array( ) ;
if( $keyword != '' ) {
	//essentially ignore/fake creating a tree if we are doing a search
	for( $i = 0 ; $i < $nrows ; $i ++ ) {
		$row_list[$i] = $i ;
		$depth_list[$i] = 0 ;
	}
...
for( $n = $pageNav->limitstart ; $n < $nrows ; $n ++ ) {
	...
	$tmp_cell = ($keyword == '' ? str_repeat( "&nbsp;&nbsp;&nbsp;", $repeat ) . "&#095&#095;|" . $repeat . "|&nbsp;" : '') . "<a href=\"" . $_SERVER['PHP_SELF'] . "?option=com_virtuemart&page=product.product_category_form&category_id=" . $category_tmp[$row_list[$n]]["category_child_id"] . "&category_parent_id=" . $category_tmp[$row_list[$n]]["category_parent_id"] . "\">" . $catname . "</a>" ;
	...
	$listObj->addCell( ps_product_category::product_count( $category_tmp[$row_list[$n]]["category_child_id"] ) . "&nbsp;<a href=\"" . $_SERVER['PHP_SELF'] . "?page=product.product_list&category_id=" . $category_tmp[$row_list[$n]]["category_child_id"] . "&option=com_virtuemart" . "\">[ " . $VM_LANG->_PHPSHOP_SHOW . " ]</a>" ) ;
	// Publish / Unpublish
	$tmp_cell = "<a href=\"" . $sess->url( $_SERVER['PHP_SELF'] . "?page=product.product_category_list&category_id=" . $category_tmp[$row_list[$n]]["category_child_id"] . "&func=changePublishState&keyword=$keyword" ) ;
	...
	if( $keyword == '' ) {
		...
		$listObj->addCell( $pageNav->orderUpIcon( $ibg, $upCondition, 'orderup', 'Order Down', $page, 'reorder' ) . '&nbsp;' . $pageNav->orderDownIcon( $ibg, $levelcounter[$category_tmp[$row_list[$n]]["category_parent_id"]], $downCondition, 'orderdown', 'Order Down', $page, 'reorder' ) ) ;
