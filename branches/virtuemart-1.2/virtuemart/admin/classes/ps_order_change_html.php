...
class ps_order_change_html {
	...
	**************************************************************************/
    ...
    global $VM_LANG;
	?><tr> 
    <td width="35%" align="right"><?php echo $VM_LANG->_('VM_ORDER_CHANGE_UPD_BILL') ?>:</td>
    ...
    <form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
  	...
  	<input type="image" title="<?php echo $VM_LANG->_('VM_UPDATE') ?>" src="<?php echo VM_THEMEURL ?>images/edit_f2.gif" border="0"  alt="<?php echo $VM_LANG->_('VM_UPDATE') ?>" />
	...
	**************************************************************************/
	...
	?>
		<tr>
  		<td align="right"><strong><?php echo $VM_LANG->_('VM_ORDER_CHANGE_UPD_SHIP') ?>:</strong></td>
  		<td align="left">
    		<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
      		<select name="ship_to">
        		<?php
	...
	**************************************************************************/
    ...
    global $VM_LANG;
		...
		?>
  		<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
  		<?php echo $VM_LANG->_('VM_PAYMENT_METHOD_LIST_DISCOUNT') ?>:
	...
	**************************************************************************/
    ...
    ?>
  		<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
  		  <?php echo $VM_LANG->_('VM_COUPON_DISCOUNT') ?>:
	...
	**************************************************************************/
    ...
    <td width="5%">
  		<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>" >		
    		<input type="image" title="<?php echo $VM_LANG->_('VM_DELETE') ?>"  
    		src="<?php echo IMAGEURL ?>ps_image/delete_f2.gif" border="0"  alt="<?php echo $VM_LANG->_('VM_DELETE') ?>" />
	...
	**************************************************************************/
    ...
    if ($product_id < 0 || $product_id == "") { 
      ...
      ?>
        ...
        <table class="adminlist">
          <tr>
            <th> <?php echo $VM_LANG->_('VM_ORDER_EDIT_ADD_PRODUCT') ?></th>
          </tr>
          <tr>
      ...
      <?php echo $html_return_parent;
         ...
         ?>
          ...
          <table class="adminlist">
            <tr>
              <th><?php echo $VM_LANG->_('VM_ORDER_PRINT_NAME') ?></th> 
              <th><?php echo $VM_LANG->_('VM_PRODUCT_FORM_CUSTOM_ATTRIBUTE_LIST') ?></th>
              <th><?php echo $VM_LANG->_('VM_PRODUCT_FORM_ATTRIBUTE_LIST') ?></th>
              <th align="left"><?php echo $VM_LANG->_('VM_ORDER_PRINT_QUANTITY') ?></th>
            ...
            <?php
              ...
              <td><input type="submit" value="<?php echo $VM_LANG->_('VM_ORDER_EDIT_ADD') ?>" /></td>
            </tr>
          </table>
      		<input type="hidden" name="page" value="order.order_print" />
      		<input type="hidden" name="option" value="com_virtuemart" />
	...
	function list_products($product_id, $skumode=false) {
		...
		if ($skumode) {
		  ...
		  $first_item = $VM_LANG->_('VM_ORDER_EDIT_CHOOSE_PRODUCT_BY_SKU');
		}
		// List all products by name
		  ...
		  $first_item = $VM_LANG->_('VM_ORDER_EDIT_CHOOSE_PRODUCT');
    }
	...
	function list_attribute($product_id, $fils=true) {
		...
		if( $db->num_rows() > 0 ) {
			...
			$display .= '<option value="-1">' . $VM_LANG->_('VM_SELECT') . '</option>';
			while ($db->next_record()) {
				$display .= '<option value="' . $db->f("product_id") . '"';
				if ($product_id == $db->f("product_id")) {
					$display .= ' selected="yes"';
