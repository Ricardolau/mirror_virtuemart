...
//vmdebug('My menus',$menus,$menu);
		...
		if($keyword or $this->showsearch){
			...
			if(is_array($customfields)){
				foreach($customfields as $k=>$f){
					...
					vmdebug('setUserState com_virtuemart.customfields.'.$k,$f);
				}
			}
	...
	public function setCanonicalLink($tpl,$document,$categoryId,$manId){
		...
		if ($format == 'html') {
			...
			$document->addHeadLink( vUri::getInstance()->toString(array('scheme', 'host', 'port')).JRoute::_($link, FALSE) , 'canonical', 'rel', '' );
