...
if( !$auth['show_prices'] || !isset($price_info["product_price_id"] )) {
	...
	$link = $sess->url( $_SERVER['PHP_SELF'].'?page=shop.ask&amp;product_id='.$product_id.'&amp;subject='. urlencode( $VM_LANG->_('VM_PRODUCT_CALL').": $product_name") );
	echo vmCommonHTML::hyperLink( $link, $VM_LANG->_('VM_PRODUCT_CALL') );
}
?>
...
if(!empty($discount_info["amount"])) {
	echo "<br />";
	echo $VM_LANG->_('VM_PRODUCT_DISCOUNT_SAVE').": ";
