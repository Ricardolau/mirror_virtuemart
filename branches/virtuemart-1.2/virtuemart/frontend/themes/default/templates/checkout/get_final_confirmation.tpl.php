...
if(!ps_checkout::noShipToNecessary()) {
	...
	echo '<tr><td valign="top"><strong>'.$VM_LANG->_('VM_ADD_SHIPTO_2') . ":</strong></td>";
	echo '<td>';
...
if(!ps_checkout::noShippingMethodNecessary()) {
	...
	echo '<tr><td valign="top"><strong>'.$VM_LANG->_('VM_INFO_MSG_SHIPPING_METHOD') . ":</strong></td>";
...
if( !isset($order_total) || $order_total > 0.00 ) {
	...
	$db->next_record();
	echo '<tr><td valign="top"><strong>'.$VM_LANG->_('VM_ORDER_PRINT_PAYMENT_LBL') . ":</strong></td>";
	echo '<td>';
	echo $db->f("name");
	echo "</td></tr>";
}
...
<br />
<div align="center">
    <?php echo $VM_LANG->_('VM_CHECKOUT_CUSTOMER_NOTE') ?>:<br />
    <textarea title="<?php echo $VM_LANG->_('VM_CHECKOUT_CUSTOMER_NOTE') ?>" cols="50" rows="5" name="customer_note"></textarea>
    <br />
    <?php
    if (PSHOP_AGREE_TO_TOS_ONORDER == '1') { ?>
		...
		$text = $VM_LANG->_('VM_I_AGREE_TO_TOS');
...
if( VM_ONCHECKOUT_SHOW_LEGALINFO == '1' ) {
    ...
    ?>
<div align="center">
<input type="submit" onclick="return( submit_order( this.form ) );" class="button" name="formSubmit" value="<?php echo $VM_LANG->_('VM_ORDER_CONFIRM_MNU') ?>" />
</div>
<?php
if(  PSHOP_AGREE_TO_TOS_ONORDER == '1' ) {
