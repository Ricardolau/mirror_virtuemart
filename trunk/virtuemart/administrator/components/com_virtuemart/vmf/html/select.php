...
abstract class vHtmlSelect
{
	...
	{
		$arr = array(vHtml::_('select.option', '0', vmText::_($no)), vHtml::_('select.option', '1', vmText::_($yes)));

		return vHtml::_('select.radiolist', $arr, $name, $attribs, 'value', 'text', (int) $selected, $id);
	}
	...
	public static function suggestionlist($data, $optKey = 'value', $optText = 'text', $idtag = null, $translate = false)
	{
		...
		JLog::add(
			'vHtmlSelect::suggestionlist() is deprecated. Create the <datalist> tag directly instead.',
			JLog::WARNING,
			'deprecated'
		);
		...
		if (!$idtag)
		{
			throw new InvalidArgumentException('$idtag is a required argument in deprecated vHtmlSelect::suggestionlist');
		}
	 ...
	 *                            Selection options. See {@see vHtmlSelect::options()}.
		...
		foreach ($data as $dataKey => $group)
		{
			...
			else
			{
				$html .= $groupIndent . '<optgroup' . (empty($id) ? '' : ' id="' . $id . '"') . ' label="'
					. ($options['group.label.toHtml'] ? vRequest::vmSpecialChars($label) : $label) . '">' . $options['format.eol']
	...
	{
		// Set default options
		$options = array_merge(vHtml::$formatOptions, array('format.depth' => 0, 'option.format' => '', 'id' => null));
		...
		return vHtml::_('select.genericlist', $data, $name, $options);
	}
	 ...
	 * @return  object
	 *
	 * @deprecated  4.0  Use vHtmlSelect::groupedList()
	 * @see     vHtmlSelect::groupedList()
	 * @since   1.5
	 */
	public static function optgroup($text, $optKey = 'value', $optText = 'text')
	{
		JLog::add('vHtmlSelect::optgroup() is deprecated, use vHtmlSelect::groupedList() instead.', JLog::WARNING, 'deprecated');
	...
	public static function options($arr, $optKey = 'value', $optText = 'text', $selected = null, $translate = false)
	{
		...
		foreach ($arr as $elementKey => &$element)
		{
			...
			{
				$html .= $baseIndent . '<optgroup label="' . ($options['list.translate'] ? vmText::_($text) : $text) . '">' . $options['format.eol'];
				$baseIndent = str_repeat($options['format.indent'], ++$options['format.depth']);
			}
			...
			else
			{
				...
				$html .= $baseIndent . '<option value="' . ($options['option.key.toHtml'] ? vRequest::vmSpecialChars($key) : $key) . '"'
					. $extra . '>';
		...
		$translate = false)
	{
		...
		{
			$k = $obj->$optKey;
			$t = $translate ? vmText::_($obj->$optText) : $obj->$optText;
