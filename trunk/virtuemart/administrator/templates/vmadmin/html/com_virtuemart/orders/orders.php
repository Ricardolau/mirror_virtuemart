...
?>
	...
	<form action="index.php?option=com_virtuemart&view=orders" method="post" name="adminForm" id="adminForm">
		...
		<div id="filterbox" class="filter-bar">
			...
			$tool['subtitle'] = vmText::_('Please first make a selection from the list');
			$tool['fields'] = array(
				...
				if (count($this->orderslist) > 0) {
					...
					foreach ($this->orderslist as $key => $order) {
						...
						<tr class="row<?php echo $k . ' status-' . strtolower($order->order_status); ?>">
							...
							<!-- Payment method -->
							<td>
								<span class="uk-label" ><?php echo $order->payment_method; ?></span>
							</td>
							<!-- Shipment method -->
							<!--				quorvia-->
							...
							?>
							<td>
								<span class="uk-label" style="<?php echo $shipmentcolorStyle ?>"><?php echo $order->shipment_method; ?></span>
							</td>
							<!-- Print view -->
							...
							<td>
								...
								?>
									...
									<div class="uk-label" style="<?php echo $colorStyle ?>">
										<?php echo vmText::_($status_name) ?>
										<span class="uk-margin-small-left" uk-icon="icon:  triangle-down"></span>
									</div>


										<div class="uk-width-large uk-form-horizontal" uk-dropdown="mode: click;animation: uk-animation-slide-bottom-small; duration: 1000">
											<?php
											...
											echo VmuikitHtml::row('textarea', 'COM_VIRTUEMART_ADD_COMMENT', 'orders[' . $order->virtuemart_order_id . '][comments]', '');
											?>
											<hr/>
