...
echo "<h2>".$VM_LANG->_('VM_PARAMETER_SEARCH')."</h2>";

	if (!$db->next_record()) { // There is no published Product Type
		echo $VM_LANG->_('VM_PARAMETER_SEARCH_BAD_PRODUCT_TYPE');
	}
	else {
		echo "<table width=\"100%\" border=\"0\">\n<tr><td width=\"40%\">";
		echo $VM_LANG->_('VM_PARAMETER_SEARCH_IN_CATEGORY').": ".$db->f("product_type_name");
		// Reset form
		...
		echo $VM_LANG->_('VM_PARAMETER_SEARCH_RESET_FORM') ."\">\n</form>";
...
<?php 
	...
	********************************************************/
		...
		while ($db->next_record()) {
			...
			if (!empty($parameter_values) && $db->f("parameter_multiselect")=="Y") {
				if ($parameter_type == "V") { // type: Multiple Values
					...
					$comp .= "<option value=\"find_in_set_all\"".(($get_item_value_comp=="find_in_set_all")?" selected":"").">".$VM_LANG->_('VM_PARAMETER_SEARCH_FIND_IN_SET_ALL')."</option>\n";
					$comp .= "<option value=\"find_in_set_any\"".(($get_item_value_comp=="find_in_set_any")?" selected":"").">".$VM_LANG->_('VM_PARAMETER_SEARCH_FIND_IN_SET_ANY')."</option>\n";
					$comp .= "</select></td>";
				}
				else { // type: all other
				...
				switch( $parameter_type ) {
					...
					case "T": // Text
						...
						$comp  = "<select class=\"inputbox\" name=\"".$item_name."_comp\">\n";
						$comp .= "<option value=\"like\"".(($get_item_value_comp=="like")?" selected":"").">".$VM_LANG->_('VM_PARAMETER_SEARCH_IS_LIKE')."</option>\n";
						$comp .= "<option value=\"notlike\"".(($get_item_value_comp=="notlike")?" selected":"").">".$VM_LANG->_('VM_PARAMETER_SEARCH_IS_NOT_LIKE')."</option>\n";
						$comp .= "<option value=\"fulltext\"".(($get_item_value_comp=="fulltext")?" selected":"").">".$VM_LANG->_('VM_PARAMETER_SEARCH_FULLTEXT')."</option>\n";
						$comp .= "</select>";
						break;
					case "S": // Short Text
			...
			if (!empty($parameter_values)) { // List of values
				...
				if ($db->f("parameter_multiselect")=="Y") {
					...
					$attr .= "\">\n";
					$attr .= "<option value=\"\">".$VM_LANG->_('VM_SELECT')."</option>\n";
					foreach($fields as $field) {
		...
		while ($db->next_record()) {
			...
			if ($parameter_type!="B") {
				...
				if ($db->f("parameter_description")) {
					echo "&nbsp;";
					echo mm_ToolTip($db->f("parameter_description"),$VM_LANG->_('VM_PRODUCT_TYPE_PARAMETER_FORM_DESCRIPTION'));
				}
				echo "&nbsp;:</strong></div>\n  </td>\n";
				...
				if (!empty($parameter_values) && $db->f("parameter_multiselect")=="Y") {
					if ($parameter_type == "V") { // type: Multiple Values
						...
						echo "<option value=\"find_in_set_all\"".(($get_item_value_comp=="find_in_set_all")?" selected":"").">".$VM_LANG->_('VM_PARAMETER_SEARCH_FIND_IN_SET_ALL')."</option>\n";
						echo "<option value=\"find_in_set_any\"".(($get_item_value_comp=="find_in_set_any")?" selected":"").">".$VM_LANG->_('VM_PARAMETER_SEARCH_FIND_IN_SET_ANY')."</option>\n";
						echo "</select></td>";
					}
					else { // type: all other
						// Multiple section List of values - no comparison
					...
					switch( $parameter_type ) {
						...
						case "T": // Text
							...
							echo "<td width=\"10%\" height=\"2\" valign=\"top\" align=\"center\">\n";
							echo "<select class=\"inputbox\" name=\"".$item_name."_comp\">\n";
							echo "<option value=\"like\"".(($get_item_value_comp=="like")?" selected":"").">".$VM_LANG->_('VM_PARAMETER_SEARCH_IS_LIKE')."</option>\n";
							echo "<option value=\"notlike\"".(($get_item_value_comp=="notlike")?" selected":"").">".$VM_LANG->_('VM_PARAMETER_SEARCH_IS_NOT_LIKE')."</option>\n";
							echo "<option value=\"fulltext\"".(($get_item_value_comp=="fulltext")?" selected":"").">".$VM_LANG->_('VM_PARAMETER_SEARCH_FULLTEXT')."</option>\n";
							echo "</select></td>";
							break;
				...
				if (!empty($parameter_values)) { // List of values
					...
					if ($db->f("parameter_multiselect")=="Y") {
						...
						echo "\">\n";
						echo "<option value=\"\">".$VM_LANG->_('VM_SELECT')."</option>\n";
						foreach($fields as $field) {
				...
				switch( $parameter_type ) {
					case "D": // Date & Time
						echo " (".$VM_LANG->_('VM_PRODUCT_TYPE_PARAMETER_FORM_TYPE_DATE_FORMAT')." ";
						echo $VM_LANG->_('VM_PRODUCT_TYPE_PARAMETER_FORM_TYPE_TIME_FORMAT').")";
						break;
					case "A": // Date
						echo " (".$VM_LANG->_('VM_PRODUCT_TYPE_PARAMETER_FORM_TYPE_DATE_FORMAT').")";
						break;
	...
	echo "<tr>\n  <td width=\"35%\" height=\"2\" valign=\"top\"><div align=\"right\"><strong>";
	echo $VM_LANG->_('VM_CART_PRICE')."&nbsp;:</strong></div>\n  </td>\n";
	// comparison
	echo "<td width=\"10%\" height=\"2\" valign=\"top\" align=\"center\">\n";
...
?>	
	...
	<tr><td colspan="3" height="2" ><div align="center">
		<input type="submit" class="button" name="search" value="<?php echo $VM_LANG->_('VM_SEARCH_TITLE') ?>">
		</div></td>
	</tr>
</table>
<?php
