			...
			if( ( $dest_country == "US") && in_array($dest_state,$states) )	{
				...
				if( function_exists( "curl_init" )) {
					...
					if( !empty( $error )) {
						...
						$vmLogger->info( $VM_LANG->_('VM_INTERNAL_ERROR')." USPS.com" );
						$error = true;
					}
					else {
						/* XML Parsing */
					...
					if( !$fp ) {
						$error = true;
						$vmLogger->debug( $VM_LANG->_('VM_INTERNAL_ERROR').": $errstr ($errno)");
					}
					else {
