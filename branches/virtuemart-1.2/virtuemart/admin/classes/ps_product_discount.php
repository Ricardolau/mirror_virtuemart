...
class ps_product_discount {
	...
	function discount_list( $discount_id = 0, $show_dates = true ) {
		...
		if( $db->num_rows() > 0 ) {
			...
			$html .= "<option id=\"*1\" value=\"0\">" . $VM_LANG->_('VM_INFO_MSG_VAT_ZERO_LBL') . "</option>\n" ;
      ...
      <a href=\"" . $_SERVER['PHP_SELF'] . "?option=$option&page=product.product_discount_form\" target=\"_blank\">" . $VM_LANG->_('VM_PRODUCT_DISCOUNT_ADDDISCOUNT_TIP') . "</a>" ;
		}
		return $html ;
	}
}
