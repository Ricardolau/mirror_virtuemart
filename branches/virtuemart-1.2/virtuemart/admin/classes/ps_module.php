		...
		if ($dir_list) {
			...
			if ($perm->check( $dir_list[ 'perms' ]) ) {
				...
				if ( !file_exists(PAGEPATH.$modulename.".".$pagename.".php") ) {
					...
					$error = $VM_LANG->_('VM_PAGE_404_1');
					$error .= ' '.$VM_LANG->_('VM_PAGE_404_2') ;
				...
				$vmLogger->err( $VM_LANG->_('VM_MOD_NO_AUTH') );
				return false;
			}
		}
		else {
			$error = $VM_LANG->_('VM_MOD_NOT_REG');
			$error .= '"'.$modulename .'" '. $VM_LANG->_('VM_MOD_ISNO_REG');
			$vmLogger->err( $error );
			return false;
		}
