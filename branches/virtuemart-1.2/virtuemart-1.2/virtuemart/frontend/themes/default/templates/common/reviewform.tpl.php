...
if (!$alreadycommented) {
	...
	function check_reviewform() {
	    ...
	    for (var i=0; i<form.user_rating.length; i++)
	       if (form.user_rating[i].checked)
	          ausgewaehlt = true;
	    if (!ausgewaehlt)  {
	      alert('<?php echo $VM_LANG->_('VM_REVIEW_ERR_RATE',false)  ?>');
	      return false;
	    }
	    else if (form.comment.value.length < <?php echo VM_REVIEWS_MINIMUM_COMMENT_LENGTH ?>) {
	      alert('<?php echo sprintf( $VM_LANG->_('VM_REVIEW_ERR_COMMENT1',false),VM_REVIEWS_MINIMUM_COMMENT_LENGTH )  ?>');
	      return false;
	    }
	    else if (form.comment.value.length > <?php echo VM_REVIEWS_MAXIMUM_COMMENT_LENGTH ?>) {
	      alert('<?php echo sprintf( $VM_LANG->_('VM_REVIEW_ERR_COMMENT2',false), VM_REVIEWS_MAXIMUM_COMMENT_LENGTH )  ?>');
	      return false;
	    }
	    else {
	      return true;
    ...
    <h4><?php echo $VM_LANG->_('VM_WRITE_REVIEW')  ?></h4>
    <br /><?php echo $VM_LANG->_('VM_REVIEW_RATE')  ?>
    <form method="post" action="<?php echo URL  ?>index.php" name="reviewForm" id="reviewform">
    <table cellpadding="5" summary="<?php echo $VM_LANG->_('VM_REVIEW_RATE') ?>">
      <tr>
        <th id="five_stars">
    ...
    <br />
    <input class="button" type="submit" onclick="return( check_reviewform());" name="submit_review" title="<?php echo $VM_LANG->_('VM_REVIEW_SUBMIT')  ?>" value="<?php echo $VM_LANG->_('VM_REVIEW_SUBMIT')  ?>" />
...
<?php
	...
	echo $VM_LANG->_('VM_REVIEW_ALREADYDONE');
}
