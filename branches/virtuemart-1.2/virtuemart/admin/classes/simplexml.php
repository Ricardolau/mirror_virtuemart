 ...
 *
 * The XML Parser extension (expat) is required to use vmSimpleXML.
 *
 ...
 * vmSimpleXML, not implicit as with PHP5. Write
 ...
 * </code>
 *
 * Note: vmSimpleXML cannot be used to access sophisticated XML doctypes
...
class vmSimpleXML {
	 ...
	 * vmSimpleXMLElement with properties containing the data held within the xml document.
	 * If any errors occur, it returns FALSE.
	 *
	 * @param string  Well-formed xml string data
	 * @param string  currently ignored
	 * @return object vmSimpleXMLElement
	 */
	 ...
	 * to an object  of class vmSimpleXMLElement. If any errors occur during file access or
	...
	/**
	 * Get a vmSimpleXMLElement object from a DOM node.
	 *
	 * This function takes a node of a DOM  document and makes it into a vmSimpleXML node.
	 * This new object can then be used as a native vmSimpleXML element. If any errors occur,
	 * it returns FALSE.
	 *
	...
	function _handleError( $code, $line, $col ) {
		$GLOBALS['vmLogger']->err( 'XML Parsing Error at ' . $line . ':' . $col . '. Error ' . $code . ': ' . xml_error_string( $code ) ) ;
	}
	...
	function _startElement( $parser, $name, $attrs = array() ) {
		...
		} else //If it isn't root level, use the stack to find the parent
		{
...
class vmSimpleXMLElement {
	 ...
	 * @param array  $attrs
	 * @param int 	 $level
	 * @return vmSimpleXMLElement 	The added child object
	 */
	...
	/**
	 * traverses the tree calling the $callback( vmSimpleXMLElement
	 * $this, mixed $args=array() ) function with each vmSimpleXMLElement.
	 *
